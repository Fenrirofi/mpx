// ╔══════════════════════════════════════════════════════════════════════════════╗
// ║  IBL Bake Shaders                                                           ║
// ║  Pass A: equirectangular HDR  →  cubemap face (6×)                         ║
// ║  Pass B: cubemap  →  diffuse irradiance cubemap (6×)                       ║
// ║  Pass C: cubemap  →  specular prefilter mip (6× per mip)                   ║
// ║  Pass D: BRDF integration LUT (once, 512×512)                              ║
// ╚══════════════════════════════════════════════════════════════════════════════╝
...
fn dir_to_equirect(d: vec3<f32>) -> vec2<f32> {
    return vec2<f32>(
        atan2(d.z, d.x) / TWO_PI + 0.5,
        asin(clamp(d.y, -1.0, 1.0)) / PI + 0.5,
    );
}

// ══════════════════════════════════════════════════════════════════════════════
//  PASS A — Equirectangular → Cubemap face
// ══════════════════════════════════════════════════════════════════════════════
...
// ══════════════════════════════════════════════════════════════════════════════
//  PASS B — Diffuse irradiance convolution
// ══════════════════════════════════════════════════════════════════════════════
@group(0) @binding(1) var t_env_cube:  texture_cube<f32>;
...
fn cs_irradiance(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    // Tangent space for hemisphere sampling
    ...
    let SAMPLES    = 64u;
    let delta      = PI / f32(SAMPLES);

    for (var phi_i = 0u; phi_i < SAMPLES * 2u; phi_i++) {
        let phi = delta * f32(phi_i);
        for (var theta_i = 0u; theta_i < SAMPLES; theta_i++) {
            let theta   = delta * f32(theta_i);
            let local   = vec3<f32>(sin(theta)*cos(phi), sin(theta)*sin(phi), cos(theta));
            let world   = local.x * right + local.y * forward + local.z * n;
            irradiance += textureSampleLevel(t_env_cube, s_linear, world, 0.0).rgb * cos(theta) * sin(theta);
        }
    }
    irradiance = irradiance * PI / f32(SAMPLES * SAMPLES);
    textureStore(out_irrad, vec2<i32>(gid.xy), vec4<f32>(irradiance, 1.0));
}

// ══════════════════════════════════════════════════════════════════════════════
//  PASS C — Specular prefilter (GGX importance sampling)
// ══════════════════════════════════════════════════════════════════════════════
...
fn cs_prefilter(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < SAMPLES; i++) {
        ...
        if nl > 0.0 {
            color        += textureSampleLevel(t_env_prefilter, s_linear, l, 0.0).rgb * nl;
            total_weight += nl;
        }
    }
    color = color / max(total_weight, 0.001);
    textureStore(out_prefilter, vec2<i32>(gid.xy), vec4<f32>(color, 1.0));
}
...
fn geometry_smith_ibl(ndv: f32, ndl: f32, roughness: f32) -> f32 {
    return geometry_schlick_ggx_ibl(ndv, roughness) * geometry_schlick_ggx_ibl(ndl, roughness);
}
...
fn cs_brdf_lut(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var i = 0u; i < SAMPLES; i++) {
        ...
        if nl > 0.0 {
            let G   = geometry_smith_ibl(ndv, nl, roughness);
            let G_v = G * vh / (nh * ndv);
    ...
    A /= f32(SAMPLES); B /= f32(SAMPLES);
    textureStore(out_lut, vec2<i32>(gid.xy), vec4<f32>(A, B, 0.0, 1.0));
}
